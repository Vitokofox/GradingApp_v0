...
def upgrade() -> None:
    ...
    if "moisture_captures" not in existing_tables:
        op.create_table(
            "moisture_captures",
    ...
    if "moisture_readings" not in existing_tables:
        op.create_table(
            "moisture_readings",
            sa.Column("id", sa.Integer(), primary_key=True),
...
def downgrade() -> None:
    # Existing installations may have had these tables before Alembic adoption.
    pass
