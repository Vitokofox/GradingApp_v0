    ...
    const handleSendEmail = async (format = 'image') => {
        ...
        try {
            const result = await emailService.sendReport({
                ...
                subject: `Reporte Grado en Línea N° ${inspection.id}`,
                body: `Reporte de Inspección - Grado en Línea\nFecha: ${new Date().toLocaleDateString()}\nPiezas: ${stats.totalPieces}\n\nVer adjunto para más detalles.`,
                format,
                filename: `GradoLinea_${inspection.id}_${new Date().toISOString().split('T')[0]}`
            });
