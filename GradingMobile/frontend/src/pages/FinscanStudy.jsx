...
// eslint-disable-next-line no-unused-vars -- framer-motion uses JSXIdentifier (<motion.div>) not tracked by no-unused-vars without eslint-plugin-react
...
export default function FinscanStudy() {
    ...
    const loadMasterData = async () => {
        try {
            ...
            setMachines(Array.isArray(ma) ? ma : []);
            setSupervisors(Array.isArray(sup) ? sup : []);
            setLengths(Array.isArray(l) ? l.sort((a, b) => parseFloat(a.name) - parseFloat(b.name)) : []);
        } catch (e) {
...
function SelectField({ label, _name, value, onChange, options, valueKey = 'id' }) {
    return (
        <div>
...
function ActiveStudyView({ study, stats, grades, currentItem, setCurrentItem, onAddItem, _lastAdded, masterData }) {
                ...
                <div className="ga-card print:hidden" style={{ height: 'fit-content' }}>
                    ...
                    <div className="ga-card__body ga-stack">
                        ...
                        <div className="u-mb-2">
                            ...
                            {masterData && Array.isArray(masterData.lengths) && masterData.lengths.length > 0 ? (
...
function StatCard({ label, value, subValue, icon, _color = "var(--ga-text)" }) {
    return (
