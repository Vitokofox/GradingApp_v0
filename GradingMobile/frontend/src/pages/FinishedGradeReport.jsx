...
import { ArrowLeft, Printer, Mail, FileText, Share2, CheckCircle } from 'lucide-react';
...
export default function FinishedGradeReport() {
    ...
    const calculateStats = (data) => {
        ...
        data.forEach(item => {
            ...
            const dName = (item.defect && item.defect.name) ? item.defect.name : 'SIN DEFECTO / EN GRADO';
    ...
    const handleSendEmail = async (format = 'image') => {
        const reportElement = document.getElementById('report-card-content');
        if (!reportElement) {
        ...
        try {
            const result = await emailService.sendReport({
                ...
                subject: `Reporte de Producto Terminado N° ${inspection.id}`,
                body: `Reporte de Inspección - Producto Terminado\nFecha: ${new Date().toLocaleDateString()}\nPiezas: ${stats.totalPieces}\n\nVer adjunto para más detalles.`,
                format,
                filename: `Producto_${inspection.id}_${new Date().toISOString().split('T')[0]}`
            });
    ...
    return (
        <div className="ga-page">
            <div style={{ maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
                ...
                <div id="report-card-content" className="ga-card" style={{ background: 'white', color: '#1e293b', padding: 0, overflow: 'hidden', boxShadow: 'none' }}>

                    {/* Cabecera del Reporte (Report Header - Corporate Grey) */}
                    <div style={{ background: 'var(--ga-primary)', padding: '1.5rem', printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <div style={{ textAlign: 'right', color: '#e0e7ff', fontFamily: 'monospace', fontSize: '0.875rem' }}>
            ...
            {showToast && (
                <div style={{
                    ...
                    <span style={{ fontSize: '0.9rem', fontWeight: 500 }}>¡Imagen copiada al portapapeles!</span>
                </div>
            )}
