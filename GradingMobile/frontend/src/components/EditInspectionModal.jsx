...
export default function EditInspectionModal({ isOpen, onClose, inspection, onUpdate }) {
    ...
    const handleResultChange = (id, newCountVal) => {
        ...
        setResults(prev => {
            ...
            // Si el target es la base, simplemente actualizamos
            if (!targetResult.defect_id) {
                return prev.map(r => r.id === id ? { ...r, pieces_count: newCount, isDirty: true } : r);
            }

            // Si es un defecto, intentamos compensar con la base (mismo grado o base global)
            // Para simplificar y ser consistente con escritorio, buscamos el "Sin Defecto" del mismo grado
            const baseResult = prev.find(r => r.grade_id === targetResult.grade_id && !r.defect_id);
            ...
            // Si no hay base en ese grado, buscamos la base global (primer grado sin defecto)
            const globalBase = prev.find(r => !r.defect_id);
            if (globalBase) {
                return prev.map(r => {
                    if (r.id === id) return { ...r, pieces_count: newCount, isDirty: true };
                    if (r.id === globalBase.id) return { ...r, pieces_count: Math.max(0, r.pieces_count - diff), isDirty: true };
                    return r;
                });
            }
