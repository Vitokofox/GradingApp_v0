...
export default function InspectionForm({ type, title }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setShifts(normalizeArray(shiftsData));
                setJourneys(normalizeArray(journeysData));
                setAreas(normalizeArray(areasData));
                setMachines(normalizeArray(machinesData));
                setMarkets(normalizeArray(marketsData));
                setProducts(normalizeArray(productsData));
                setStates(normalizeArray(statesData));
                setTerminations(normalizeArray(terminationsData));
                setOrigins(normalizeArray(originsData));
                setSupervisors(normalizeArray(supervisorsData));

            } catch (error) {
                console.error("Error creating inspection", error);
                alert("Error cargando datos maestros. Verifique su conexión o sincronización.");
            } finally {
    ...
    const renderSelect = (label, name, options, icon = null, optionsLabelKey = 'name') => (
        <div>
            ...
            >
                ...
                {normalizeArray(options).map(opt => (
    ...
    if (type === 'finished_product' && !inspectionSubtype) {
        return (
            <div className="ga-stack" style={{ padding: '1rem' }}>
                <h2 className="u-bold u-text-center u-mb-6" style={{ fontSize: '1.25rem', color: 'var(--ga-primary)' }}>
                    Tipo de Inspección Terminado
                </h2>
                <div className="ga-grid" style={{ gridTemplateColumns: '1fr' }}>
                    <motion.div
                        whileTap={{ scale: 0.95 }}
                        onClick={() => setInspectionSubtype('lote')}
                        className="ga-card u-p-6 u-center ga-stack"
                        style={{ background: 'var(--ga-surface)', border: '2px solid var(--ga-border)', cursor: 'pointer' }}
                    >
                        <Tag size={40} className="u-color-primary u-mb-3" />
                        <span className="u-bold" style={{ fontSize: '1.1rem' }}>Terminado Lote</span>
                        <p className="u-muted u-text-xs">Paquete ya tarjeteado y enzunchado</p>
                    </motion.div>

                    <motion.div
                        whileTap={{ scale: 0.95 }}
                        onClick={() => setInspectionSubtype('linea')}
                        className="ga-card u-p-6 u-center ga-stack"
                        style={{ background: 'var(--ga-surface)', border: '2px solid var(--ga-border)', cursor: 'pointer' }}
                    >
                        <Layout size={40} className="u-color-accent u-mb-3" />
                        <span className="u-bold" style={{ fontSize: '1.1rem' }}>Terminado en Línea</span>
                        <p className="u-muted u-text-xs">Revisión de piezas sueltas (Stacker)</p>
                    </motion.div>
                </div>
            </div>
        );
        ...
        <div style={{ maxWidth: '1024px', margin: '0 auto', position: 'relative' }}>
            {isLoadingData && (
                <div style={{
                    ...
                    <div className="ga-spinner" style={{ width: '40px', height: '40px', border: '4px solid var(--ga-primary)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                    <p className="u-mt-4 u-bold u-muted">Cargando datos maestros...</p>
                </div>
            )}
            <motion.div
            ...
            >
                <div className="ga-card__header">
                    ...
                    {inspectionSubtype && (
                        <button
                            onClick={() => { setInspectionSubtype(null); setFormData(prev => ({ ...prev, lot: '' })) }}
                                ...
                                {inspectionSubtype === 'linea' ? (
                                    <div>
                                        ...
                                        <select
                                            ...
                                            value={formData.lot}
                                            onChange={handleChange}
                                            className="ga-control u-bold u-text-primary"
                                        >
                                            <option value="">Seleccione Stacker...</option>
